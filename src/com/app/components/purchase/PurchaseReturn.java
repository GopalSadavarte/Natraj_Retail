...
public final class PurchaseReturn extends AbstractButton implements CellEditorListener {
    ...
    public PurchaseReturn(JMenuItem currentMenu, HashMap<String, JInternalFrame> frameTracker,
            HashMap<JInternalFrame, String> frameKeyMap, final JDialog dialog) {

        super("Purchase Return Entry", currentMenu, frameTracker, frameKeyMap, dialog);
    ...
    private void saveIntoDB(Boolean isNewEntry) throws Exception {
        ...
        if (affectedRows > 0) {
            ...
            for (int row = 0; row < rows; row++) {
                ...
                query = "select id from products where p_id = ? limit 1";
                pst = DBConnection.con.prepareStatement(query);
